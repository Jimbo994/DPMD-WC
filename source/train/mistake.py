...
def softplus100(features):
    return(tf.log(1.0 + tf.exp(100.*features))/100.)
...
#        l2_atom_ener_loss = tf.reduce_mean (tf.square(atom_ener_hat_reshape - atom_ener_reshape), name = "l2_atom_ener_" + suffix)
        l2_atom_ener_loss = tf.reduce_mean (tf.square(atom_ener_reshape - atom_ener_reshape), name = "l2_atom_ener_" + suffix)
    ...
    def build_interaction (self, 
        ...
        energy_raw = tf.reshape(atom_ener, [-1, natoms[0]/3], name = 'atom_energy_'+suffix)
    ...
    def build_atom_net (self,
        ...
        self._message(print(tf.Print(natoms, [natoms])))
        self._message("dimensions of natoms is:  %s" % print(tf.Print(natoms, [natoms])))
        self._message("dimensions of inputs is:  %s" % print(tf.Print(inputs, [inputs])))
...
#        #Print statement adjustments
            self._message("dimensions of inputs_i is:  %s" % print(tf.Print(inputs_i, [inputs_i])))
        #End print statement
...
#                                [shape_i[1], outputs_size],
#                                global_tf_float_precision,
#                                tf.random_normal_initializer(stddev=stddev/np.sqrt(shape_i[1]+outputs_size), seed = seed))
...
#                                tf.random_normal_initializer(stddev=stddev, mean = bavg, seed = seed))
            ...
            #self._message("dimensions of w is:  %s" % print(tf.Print(w, [w])))
            b = tf.get_variable('bias', 
                                [outputs_size], 
                                global_tf_float_precision,
                                tf.random_normal_initializer(stddev=stddev, mean = bavg, seed = seed))
            hidden = tf.matmul(inputs, w) + b
            #hidden = tf.matmul(inputs_i, w) + b
